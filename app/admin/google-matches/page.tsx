...
export default function GoogleMatchesPage() {
  ...
  return <main className="min-h-screen bg-background text-foreground"><Navbar /><section className="mx-auto max-w-6xl px-4 py-10">
    <p className="text-sm font-medium text-cyan-400">Admin workspace</p>
    <div className="mt-1 flex flex-wrap items-center justify-between gap-4"><h1 className="text-4xl font-bold">Canonical Google matches</h1><button type="button" disabled={refreshingRatings} onClick={() => void refreshRatings()} className="inline-flex items-center gap-2 rounded-lg border border-cyan-400/40 px-3 py-2 text-sm font-medium text-cyan-300 transition hover:bg-cyan-400/10 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${refreshingRatings ? "animate-spin" : ""}`} />{refreshingRatings ? "Refreshing…" : "Refresh 10 Google ratings"}</button></div>
