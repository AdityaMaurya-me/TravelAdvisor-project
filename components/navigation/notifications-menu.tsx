...
type Notification = { id: string; candidate_id: string | null; title: string; body: string | null; is_read: boolean; created_at: string };
...
export function NotificationsMenu({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const load = async () => {
    ...
    const { data } = await (supabase as any).from("user_notifications").select("id,candidate_id,title,body,is_read,created_at").order("created_at", { ascending: false }).limit(12);
  ...
  const openNotification = async (notification: Notification) => {
    ...
    onOpenChange(false);
    router.push(`/curation/locations${notification.candidate_id ? `?request=${notification.candidate_id}` : ""}`);
  };
