...
const styleUrl = mapTilerKey ? `https://api.maptiler.com/maps/streets-v2/style.json?key=${mapTilerKey}` : undefined;
  ...
  useEffect(() => {
    ...
    const initialise = async () => {
      try {
        ...
        if (!response.ok) throw new Error(response.status === 401 || response.status === 403 ? "MapTiler rejected this browser key." : "Map style request failed.");
        ...
        if (!disposed && !controller.signal.aborted) setMapError(caught instanceof Error ? caught.message : "The route map could not load.");
      }
    };
    void initialise();
    return () => { disposed = true; controller.abort(); markers.forEach((marker) => marker.remove()); map?.remove(); };
  }, [bounds, geometry, points]);
