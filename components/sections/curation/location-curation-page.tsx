...
export function LocationCurationPage() {
  ...
  const load = async () => {
    const [{ data: userResult }, { data: cities }, { data: role }, { data: drafts, error }] = await Promise.all([
      ...
      (supabase as any).from("location_candidates").select("id, created_by, name, source, status, created_at, proposed_categories, destination_id, source_url, source_reference, latitude, longitude, description, image_url, image_attribution, review_notes, destination:places!location_candidates_destination_id_fkey(name)").order("created_at", { ascending: false }),
    ]);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await (supabase as any).from("location_candidates").insert({
      ...
      proposed_categories: form.categories.split(",").map((item) => item.trim().toLowerCase()).filter(Boolean),
    ...
    if (error) { setMessage(error.message); return; }
    setForm(emptyForm);
    setMessage(isCurator ? "Candidate saved to the curator queue." : "Thanks — your location request is now waiting for review.");
    await load();
  };
