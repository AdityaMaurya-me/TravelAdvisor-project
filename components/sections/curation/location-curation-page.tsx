...
export function LocationCurationPage() {
  ...
  const [isCurator, setIsCurator] = useState(false); const [isAdmin, setIsAdmin] = useState(false);
  ...
  const load = async () => {
    ...
    setIsCurator(Boolean(role?.role)); setIsAdmin(role?.role === "admin");
